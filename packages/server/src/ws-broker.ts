...
function createEnvelope(channel: string, type: string, payload: unknown, requestId?: string): string {
...
export const wsBrokerPlugin = fp(async function wsBroker(fastify: FastifyInstance) {
	...
	function send(socket: WebSocket, channel: string, type: string, payload: unknown, requestId?: string) {
	...
	function handleMessage(socket: WebSocket, raw: string) {
		...
		if (handler) {
			...
			send(socket, "system", "error", { message: `Unknown channel: ${envelope.channel}` }, envelope.requestId);
		}
	}
